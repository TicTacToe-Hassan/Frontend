...
import { capGame } from "../Slices/GameSlice";
...
export const Messenger: React.FC = () => {
  ...
  const [marker, setMarker] = useState<any>(0);
  ...
  const [countp1, setCountp1] = useState<any>(0);
  const [countp2, setCountp2] = useState<any>(0);
  ...
  const showWinner = () =>
    // let message: string = [...privateChats.keys()].slice(1).toString();
    toast("Player:" + [...privateChats.keys()].slice(1).toString(), {
  ...
  useEffect(() => {
    ...
    console.log("cap" + gameInfo.game.cap);
    ...
    if (playerMove) {
      if (boardInfo.board?.gameOver1) {
        //count2++;
        setCountp2(countp2 + 1);
      }
      if (boardInfo.board?.gameOver2) {
        //count1++;
        setCountp1(countp1 + 1);
      }
    } else {
      if (boardInfo.board?.gameOver2) {
        // count2++;
    ...
    if (gameInfo.game.cap == 1) {
      setMarker(1);
    } else if (gameInfo.game.cap == 3) {
      setMarker(2);
    } else if (gameInfo.game.cap == 5) {
      setMarker(3);
    } else if (gameInfo.game.cap == 7) {
      setMarker(4);
    }
    if (countp1 >= marker) {
      showWinner();
    } else if (countp2 >= marker) {
    }
    console.log("count1 " + count1);
    console.log("count2 " + count2);
    console.log("This is cap " + gameInfo.game.cap);
  }, [boardInfo.board, userData.connected, playerMove, gameInfo]);
  ...
  const onPrivateMessage = (payload: { body: string }) => {
    ...
    if (payloadData.status == "ID") {
      ...
      // dispatch(capGame(parseInt(payloadData.message.cap)));
      dispatch(joinGame(id));
      gameInfo.game.cap = payloadData.message.cap;
      dispatch(capGame(payloadData.message.cap));
    }
          ...
          <div className="chat-box">
            ...
            {tab !== "CHATROOM" && (
              ...
              >
                ...
                <>
                  {countp1 >= marker || countp2 >= marker ? (
                    countp1 >= marker ? (
                      <div>
                        <ToastContainer />
                        <h2>
                          Player: {[...privateChats.keys()].slice(1).toString()}{" "}
                          Wins{" "}
                        </h2>
                      </div>
                    ) : (
                      <div>
                        <ToastContainer />
                        <h2> Player: {userData.username} Wins </h2>
                      </div>
                    )
                  ) : (
                    <div className="container">
                      ...
                      <div>
                        <h2>
                          Player: {userData.username} Wins : {countp2}
                        </h2>
                        <h2>
                          Player: {[...privateChats.keys()].slice(1).toString()}
                          Wins : {countp1}{" "}
